<template>
  <v-container fluid>
    ...
    >
      ...
      <template #[`top`]>
        <v-toolbar flat>
          <v-btn class="ma-1" color="primary" :to="{ name: 'category.create' }"
            ><v-icon left>mdi-plus</v-icon>New category</v-btn
          >
        ...
        <v-tooltip top>
          <template #activator="{ on, attrs }">
            <v-btn
              ...
              icon
              small
              :to="{ name: 'category.show', params: { category: item._id } }"
              v-on="on"
        ...
        <v-tooltip top>
          <template #activator="{ on, attrs }">
            <v-btn
              ...
              icon
              small
              :to="{ name: 'category.edit', params: { category: item._id } }"
              v-bind="attrs"
              v-on="on"
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      let url = `/categories?page=${this.options.page}&rowsPerPage=${
    ...
    onConfirmDelete() {
      this.deleting = true;
      api
        .delete(`/categories/${this.deleteItem}`)
        .then((response) => {
          this.fetchData();
            this.$store.dispatch("pushNotification", {
              message: response.data.message,
