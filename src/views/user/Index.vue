<template>
  <v-container fluid>
    ...
    >
      ...
      <template #[`top`]>
        <v-toolbar flat>
          <v-btn class="ma-1" color="primary" :to="{ name: 'user.create' }"
            ><v-icon left>mdi-plus</v-icon>New user</v-btn
          >
          <v-btn class="ma-1" color="green" dark @click="exportExcel()"
            ><v-icon left>mdi-export</v-icon>Export excel</v-btn
          >
          <v-btn class="ma-1" color="error" :to="{ name: 'user.trash' }"
            ><v-icon left>mdi-delete</v-icon>User trash</v-btn
          >
        ...
        <v-tooltip top>
          <template #activator="{ on, attrs }">
            <v-btn
              ...
              icon
              small
              :to="{ name: 'user.edit', params: { user: item._id } }"
              v-bind="attrs"
              v-on="on"
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      //   },
      // }
      let url = `/users?page=${this.options.page}&rowsPerPage=${
        this.options.itemsPerPage === -1
          ? this.dataTable.totalItems
          : this.options.itemsPerPage
      }`;
    ...
    onConfirmDelete() {
      this.deleting = true;
      api
        .delete(`/users/softDelete/${this.deleteItem}`)
        .then((response) => {
          this.fetchData();
          //   this.$store.dispatch("pushNotification", {
          //     message: response.data.message,
