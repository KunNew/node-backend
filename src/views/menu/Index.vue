  ...
  <v-container fluid>
    <v-card class="mb-3">
      ...
      <template v-else>
        ...
        <v-card-subtitle>List all menus.</v-card-subtitle>
      </template>
    </v-card>
    ...
    >
      ...
      <template #[`top`]>
        <v-toolbar flat>
          <v-btn class="ma-1" color="primary" :to="{ name: 'menu.create' }"
            ><v-icon left>mdi-plus</v-icon>New menu</v-btn
          >
      ...
      <template #[`item.price`]="{ item }" :ref="item.id">
        <!-- <v-badge color="green" content="$">
        
        </v-badge> -->
        {{ decimalNumber(item.price, 2) }}
      </template>
        ...
        <v-tooltip top>
          <template #activator="{ on, attrs }">
            <v-btn
              ...
              icon
              small
              :to="{ name: 'menu.show', params: { menu: item._id } }"
              v-on="on"
        ...
        <v-tooltip top>
          <template #activator="{ on, attrs }">
            <v-btn
              ...
              icon
              small
              :to="{ name: 'menu.edit', params: { menu: item._id } }"
              v-bind="attrs"
              v-on="on"
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      let url = `/menus?page=${this.options.page}&rowsPerPage=${
