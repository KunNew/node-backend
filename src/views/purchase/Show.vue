  ...
  <v-container fluid>
    <v-card class="mb-3">
      ...
      <template v-else>
        ...
        <v-card-subtitle>List all purchase.</v-card-subtitle>
      </template>
    </v-card>
    ...
    >
      ...
      <template #[`top`]>
        <v-toolbar flat>
          <v-btn class="ma-1" color="primary" :to="{ name: 'purchase.index' }"
      ...
      <template #[`item.date`]="{ item }" :ref="item.id">
        {{ item.date | filterDate }}
      </template>

      <template #[`item.total`]="{ item }" :ref="item.id">
        <v-badge content="$" color="green">
          {{ decimalNumber(item.total, 2) }}
        </v-badge>
      </template>
        ...
        <v-tooltip top>
          <template #activator="{ on, attrs }">
            <v-btn
              ...
              icon
              small
              :to="{ name: 'purchase.detail', params: { purchase: item._id } }"
              v-on="on"
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      let url = `/purchases?page=${this.options.page}&rowsPerPage=${
