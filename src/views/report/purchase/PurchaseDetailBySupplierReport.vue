<template>
  <v-container fluid>
    ...
    >
      <v-card-title>Purchase Detail By Supplier Report</v-card-title>
    </v-card>
    <v-card class="mb-3">
      <v-divider></v-divider>
      <v-card-text>
        ...
        <div id="print-area" class="report-body">
          <v-card-text class="text-center">
            ...
            <div v-if="showPart.header" class="header">
              <div class="report-name">Purchase Detail By Supplier Report</div>
              ...
              <tbody>
                <template v-for="(doc, index) in reportData">
                  <tr :key="index">
                    ...
                    <td colspan="4"></td>
                    <td class="text-right">
                      <b>{{ decimalNumber(doc.totalAmount, 2) }}</b>
                    </td>
                  </tr>
                  <tr v-for="(item, i) in doc.details" :key="index + '-' + i">
                    ...
                    <td class="text-right">
                      {{ decimalNumber(item.cost, 2) }}
                    </td>
                    <td class="text-right">{{ decimalNumber(item.qty, 0) }}</td>
                    <td class="text-right">
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      api
        .post("reports/purchaseDetailBySupplier", this.form.data)
        .then((response) => {
